...
def jaro_distance_relacionadas(s1, s2) :
    ...
    for i in range(len1) : 
        ...
        for j in range( max(0, i - max_dist),
            ...
            #print(s1[i],s2[j])
            # If there is a match or is contain in a bag of sinomys of tk
            if ((s1[i] in sinH[j] or s2[j] in sinT[i]) and hash_s2[j] == 0) : 
                print(s1[i],s2[j],"sinonimos")
                hash_s1[i] += 1; 
                hash_s2[j] += 1; 
                match += 1; 
...
#for i in range(5):
    ...
    tp1,tp2=jaro_distance(ut.get_lemmas_(t_clean_m,nlp), ut.get_lemmas_(h_clean_m,nlp))
    new_data['Jaro-Winkler_rit'].append(tp1)
    new_data['c_estructura'].append(tp2)
    
    tp1,tp2=jaro_distance_relacionadas(ut.get_lemmas_(t_clean_m,nlp), ut.get_lemmas_(h_clean_m,nlp))
    new_data['Jaro-Winkler_relacionadas'].append(tp1)
    new_data['c2_estructura'].append(tp2)

    tp1,tp2=jaro_distance_contra(ut.get_lemmas_(t_clean_m,nlp), ut.get_lemmas_(h_clean_m,nlp))
